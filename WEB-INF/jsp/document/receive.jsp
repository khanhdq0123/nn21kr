...
<script type="text/javascript">
	$(document).ready(function() {
		...
		// 그리드 
		jQuery("#list1").jqGrid({
			url: "/document/receiveDocListJson",
			datatype: "local",
		   	colNames: ['제목','발신처','발신인','수신 일시','doc_no'],
			colModel: [
				{ name: 'title_txt', 	index: 'title_txt',	align: 'left', formatter : linkFormatter, unformat : linkUnFormatter, sortable: true, width: '220'}, 
				{ name: 'vendor_nm', 	index: 'staff_nm',	align: 'center', sortable: true, width: '100' },
				{ name: 'sender_nm', 	index: 'staff_nm',	align: 'center', sortable: true, width: '80' },
				{ name: 'reg_dt',		index: 'reg_dt',	align: 'center', sortable: true, width: '100' },
				{ name: 'doc_no',		index: 'doc_no',	align: 'center', sortable: true, width: '100', hidden:true }
			],
			autowidth: true,
			height: 500,
// 			pager: '#pager1',
...
// 			},
		...
		function linkFormatter(cellVal, options, rowObj) {
			var linkURL = '<a href="javascript:CenterOpenWindow(\'/document/docDetailView?id='
					+ rowObj['doc_no']
		...
		$("#btnNewPost").click(function() {
			...
			var params = "?mode=new"
			CenterOpenWindow("/document/docDetail" + params, "_blank", width, height, "scrollbars=yes, status=no");
		});
...
// 		});
	...
	var fnBtnBind = function(){
		$("#btnMoveReceiveDoc").on("click",function(){
			saveGridSelected("list1", 'selected', 'save', "/document/updateDocListJson");
		});
	}
...
<body class="sky">
	...
	<div class="content_wrap">
		<div class="history">
			<h3 class="header">받은문서</h3>
			
			<span class="bullet6">문서관리 &gt; 받은문서</span>
		</div>
		<!-- product_admin -->
		<div class="product_admin">
			...
			<div class="report">
				<div class="min_btn">
					<input type="button" class="btn btn-primary btn-sm" value="문서발송" id="btnNewPost">
					<input type="button" class="btn btn-danger btn-sm" value="보관문서로 이동" id="btnMoveReceiveDoc">
				</div>
			</div>
			<!-- grid -->
			<div class="grid">
