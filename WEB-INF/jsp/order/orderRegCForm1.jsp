...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function fnCreateNewPartner(){
			CenterOpenWindow('/partner/partnerReg4Customer', 'open', '1200', '720', 'scrollbars=yes, status=no');
		}
		</script>
						...
						<tbody>
							<tr>
								...
								<c:choose>
									...
									<c:otherwise>
										<input type="text" id="send_nm" name="send_nm" class="readonly" placeholder="발송화주선택" onclick="fnSelectLayer('발송화주 선택','selectCustomerLayerPop','send_no','send_nm'); " style="width:86%;" readonly />													
										<input type="button" class="btn btn-success btn-xs" value="new" onclick="fnCreateNewPartner();">
									</c:otherwise>
								</c:choose>
								</td>							
							</tr>
							<c:if test="${inout_typ == '1'}">
							<tr>
...
<!-- 									<input type="text" id="receive_nm" name="receive_nm" class="readonly" placeholder="발송화주선택" onclick="fnSelectLayer('수화화주 선택','selectCustomerLayerPop','receive_no','receive_nm'); " style="width:98%;" readonly /> -->
								...
								<c:choose>
									<c:when test="${inout_typ eq '2'}">
										<input type="text" id="receive_nm" name="receive_nm" class="readonly" placeholder="발송화주선택" onclick="fnSelectLayer('수화화주 선택','selectCustomerLayerPop','receive_no','receive_nm'); " style="width:98%;" readonly />
									</c:when>
									<c:otherwise>
										<input type="text" id="receive_nm" name="receive_nm" class="readonly" placeholder="발송화주선택" onclick="fnSelectLayer('수화화주 선택','selectCustomerLayerPop','receive_no','receive_nm'); " style="width:86%;" readonly />													
										<input type="button" class="btn btn-success btn-xs" value="new" onclick="fnCreateNewPartner();">
									</c:otherwise>
								</c:choose>									
								</td>							
							</tr>
							<c:if test="${inout_typ == '2'}">
							<tr>
...
<div class="layer-mask" style="display:none;"></div>
	...
	<!-- 회주선택 -->
	<div class="lypopWrap" style="width:580px; max-height:400px; overflow-y:auto; display:none; " id="selectCustomerLayerPop" >
		<!-- header -->
		<div class="header">
		...
		<div class="contents select-layer1">
			<ul>
				<c:forEach var="entity" items="${customerList}">
					...
					[<c:out value="${entity.vendor_cd}"/>] <c:out value="${entity.vendor_nm}"/> <br />
					<c:out value="${entity.road_basadd_txt}"/>
