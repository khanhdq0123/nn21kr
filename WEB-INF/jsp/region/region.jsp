...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#btnReg").click( function() {
			if ($('#code').val()=='') {
				toast("육상포트를 선택하고 신규 등록을 해 주세요.");
				return false;
			}
			CenterOpenWindow('/region/regionDetailPop?code='+ $("#code").val(), '육상포트등록', '680','320', 'scrollbars=no, status=yes');
		});
			...
			success: function(data) {
				...
				$.each(data.rows, function(key, d) {
					...
					if(beforeUpcode != d.up_code){						
						...
						regionHtml += "<span class='mb10' onclick=\"fnRegionLoad('" + d.up_code + "','" + d.up_region_nm + "')\">" + d.up_region_nm + "</span>";
					...
					regionHtml += "<span class=\"tit\"  onclick=\"fnRegionCityLoad('" + d.code + "','" + d.region_nm + "')\">" + d.region_nm + " " + d.region_enm + "</span>";
					regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegionDetailPop('" + d.code + "',2)\">수정</span></span>";
					regionHtml += "</span>";
	...
	function fnRegionCityLoad(code,region_nm){
			...
			success: function(data) {
				...
				$.each(data.rows, function(key, d) {
					...
					if(beforeUpcode != d.up_code){						
						...
						regionHtml += "<span onclick=\"fnRegionLoad('" + d.up_code + "','" + d.up_region_nm + "')\">" + d.up_region_nm + "</span>";
						regionHtml += "<div class='mt5'>";
						...
						regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegRegion('" + d.up_code + "',4);\">도시등록</span></span>";
						regionHtml += "</th>";
						regionHtml += "<td>";
						i++;
					}
					...
					if(d.region_nm != null){
						regionHtml += "<span class=\"nation\">";						
						regionHtml += "<span class=\"tit\" onclick=\"fnRegionVillageLoad('" + d.code + "','" + d.region_nm + "')\">" + d.region_nm + " " + d.region_enm + "</span>";
						regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegionDetailPop('" + d.code + "',4)\">수정</span></span>";
						regionHtml += "</span>";
					}
	...
	function fnRegionVillageLoad(code,region_nm){
		...
		$.ajax({
			...
			success: function(data) {
				...
				regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegRegion('" + code + "',5);\">육상포트등록</span></span>";
				regionHtml += "</th>";
				...
				$.each(data.rows, function(key, d) {
					regionHtml += "<span class=\"nation\">";
					regionHtml += "<span class=\"tit\" onclick=\"fnRegionVillageLoad2('" + d.code + "','" + d.region_nm + "')\">" + d.region_nm + " " + d.region_enm + "</span>";
...
// 				regionHtml +="<tr><th>";
// 				regionHtml +="<button id=\"btnMod2\" name=\"btnMod2\" class=\"sky\" onclick=\"fnRegRegion(" + code + ", 5);\" style=\"margin-top:0; width:230px;\">+ 육상포트추가</button></th>";
// 				regionHtml +="<td>";
// 				regionHtml +="</td></tr>";
	...
	function fnRegionVillageLoad2(code,region_nm){
		...
		$.ajax({
			...
			success: function(data) {
				...
				regionHtml += "<th class=\"cont\">";
				regionHtml += "<span onclick=\"fnRegionLoad('" + code + "','" + region_nm + "')\">" + region_nm + "</span>";
				regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegRegion('" + code + "',6);\">육상포트등록</span></span>";
				regionHtml += "</th>";
				...
				$.each(data.rows, function(key, d) {
					regionHtml += "<span class=\"nation\">";
					regionHtml += "<span class=\"tit\">" + d.region_nm + " " + d.region_enm + "</span>";
...
// 				regionHtml +="<tr><th>";
// 				regionHtml +="<button id=\"btnMod2\" name=\"btnMod2\" class=\"sky\" onclick=\"fnRegRegion(" + code + ", 6);\" style=\"margin-top:0; width:230px;\">+ 육상포트추가</button></th>";
// 				regionHtml +="<td>";
// 				regionHtml +="</td></tr>";
	...
	function fnPreView(){
		CenterOpenWindow('/region/selectRegionPop?code=10000', '육상포트선택', '1000','560', 'scrollbars=no, status=yes');
	}
	...
	function fnCallbackRegionUpdate(levels){
		...
		if(levels == '1' || levels == '2'){
			...
			fnRegionCityLoad(_countryNo,_country);
		} else if(levels == '5'){
	...
	function fnRegRegion(code, levels){
		CenterOpenWindow('/region/regionRegFormPop?code='+ code + '&levels=' + levels, '육상포트등록', '680','260', 'scrollbars=no, status=yes');
	}
	
	//수정 화면
	function fnRegionDetailPop(code, levels){
		CenterOpenWindow('/region/regionDetailPop?code='+ code + '&levels=' + levels, '육상포트등록', '680','260', 'scrollbars=no, status=yes');
	}
...
.bg-color-pastel1{background-color:#ffafb0 !important;}
.bg-color-pastel2{background-color:#c4f4fe !important;}
.bg-color-pastel3{background-color:#afffba !important;}
.bg-color-pastel4{background-color:#fcffb0 !important;}
.bg-color-pastel5{background-color:#caa6fe !important;}
.bg-color-pastel6{background-color:#aee4ff !important;}
.bg-color-pastel7{background-color:#afffba !important;}

.bg-color-pastel11{background-color:#aee4ff !important;}
.bg-color-pastel12{background-color:#b5c7ed !important;}
.bg-color-pastel13{background-color:#c4f4fe !important;}
.bg-color-pastel14{background-color:#bee9b4 !important;}
.bg-color-pastel15{background-color:#fdfa87 !important;}
.bg-color-pastel16{background-color:#fcc6f7 !important;}
.bg-color-pastel17{background-color:#caa6fe !important;}
...
<body class="sky">
	...
	<div class="content_wrap">
		<div class="history">
			<h3 class="header">육상포트 관리</h3>
			
			<span class="bullet6">육상포트 관리 &gt; 육상포트 관리</span>
		</div>
		...
		<div class="corner_tab mt30" style="display:none;">
			<div class="grid" min="400">
				<div class="outcome info" style="height:30px;">
					<span class="res_search" >하위 육상포트 리스트 </span>
