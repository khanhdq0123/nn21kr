...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		fnSelectNation();
		fnRegionNationLoad(40000);


		$("#btnReg").click( function() {
			if ($('#code').val()=='') {
				toast("항공포트를 선택하고 신규 등록을 해 주세요.");
				return false;
			}
			CenterOpenWindow('/region/regionDetailPop?code='+ $("#code").val(), '항공포트등록', '680','320', 'scrollbars=no, status=yes');
		});
	...
	function fnRegionNationLoad(code){
		...
		$.ajax({
			...
			success: function(data) {
				...
				regionHtml +="<button id=\"btnMod2\" name=\"btnMod2\" class=\"sky\" onclick=\"fnRegRegion(40000, 1);\" style=\"margin-top:0; width:230px;\">+ 대륙추가</button></th>";
				regionHtml +="<td>";
				regionHtml +="</td></tr>";
	...
	function fnRegionVillageLoad(code,region_nm){
		...
		$.ajax({
			...
			success: function(data) {
				...
				regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegRegion('" + code + "',5);\">항공포트등록</span></span>";
				regionHtml += "</th>";
...
// 				regionHtml +="<tr><th>";
// 				regionHtml +="<button id=\"btnMod2\" name=\"btnMod2\" class=\"sky\" onclick=\"fnRegRegion(" + code + ", 5);\" style=\"margin-top:0; width:230px;\">+ 항공포트추가</button></th>";
// 				regionHtml +="<td>";
// 				regionHtml +="</td></tr>";
	...
	function fnRegionVillageLoad2(code,region_nm){
		...
		$.ajax({
			...
			success: function(data) {
				...
				regionHtml += "<th class=\"cont\">";
				regionHtml += "<span onclick=\"fnRegionLoad('" + code + "','" + region_nm + "')\">" + region_nm + "</span>";
				regionHtml += "<span class='minBtn'><span class='' onclick=\"fnRegRegion('" + code + "',6);\">항공포트등록</span></span>";
				regionHtml += "</th>";
...
// 				regionHtml +="<tr><th>";
// 				regionHtml +="<button id=\"btnMod2\" name=\"btnMod2\" class=\"sky\" onclick=\"fnRegRegion(" + code + ", 6);\" style=\"margin-top:0; width:230px;\">+ 항공포트추가</button></th>";
// 				regionHtml +="<td>";
// 				regionHtml +="</td></tr>";
	...
	function fnPreView(){
		CenterOpenWindow('/region/selectRegionPop?code=40000', '항공포트선택', '1000','560', 'scrollbars=no, status=yes');
	}
	...
	function fnRegRegion(code, levels){
		CenterOpenWindow('/region/regionRegFormPop?code='+ code + '&levels=' + levels, '항공포트등록', '680','260', 'scrollbars=no, status=yes');
	}
	
	//수정 화면
	function fnRegionDetailPop(code, levels){
		CenterOpenWindow('/region/regionDetailPop?code='+ code + '&levels=' + levels, '항공포트등록', '680','260', 'scrollbars=no, status=yes');
	}
...
<body class="sky">
	...
	<div class="content_wrap">
		<div class="history">
			<h3 class="header">항공포트 관리</h3>
			
			<span class="bullet6">항공포트 관리 &gt; 항공포트 관리</span>
		</div>
		...
		<div class="corner_tab mt30" style="display:none;">
			<div class="grid" min="400">
				<div class="outcome info" style="height:30px;">
					<span class="res_search" >하위 항공포트 리스트 </span>
