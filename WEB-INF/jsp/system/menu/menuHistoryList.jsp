...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		jQuery("#staffGrid").jqGrid({
			url:'/system/menu/menuHistoryListJson.do', 		  
			datatype: "local",				   
			colNames:['번호', '메뉴명', '구분', '변경내용', '처리자 ID', '처리일자', '처리시간'],
			colModel:[
				{name:'rownum' 	   	, index:'rownum'     	, width:120, align:"center"	, sortable:true, editable:false},
				{name:'menu_nm'  	, index:'menu_nm'  		, width:180, align:"center"	, sortable:true, editable:false},
				{name:'his_type' 	, index:'his_type' 		, width:250, align:"center" , sortable:true, editable:false, formatter:"select", edittype:"select", editoptions:{value:"0:등록;1:변경;2:삭제"}},
				{name:'his_content' , index:'his_content' 	, width:250, align:"center" , sortable:true, editable:false},
				{name:'regstaff_id' , index:'regstaff_id' 	, width:250, align:"center" , sortable:true, editable:false},
...
// 			rowList:[20,50,100],
		...
		$('#staff_id, #menu_id').keydown(13, function(event) {
			if (event.keyCode == 13) {
				$("#btnSearch").click();
			}
		});
...
// 		});
		...
		$("#searchForm").validate({
			rules: {
				staff_id: {maxlength: 20},
				menu_id: {maxlength: 20},
				process_dt_s: {dpDate: true},
				process_dt_e: {dpDate: true, dpCompareDate:{notBefore: '#process_dt_s'}}
			},
			messages: {
				staff_id: {maxlength: jQuery.format("처리자ID는 최대 {0} 글자 이하로 입력하세요.")},
										...
										<td>
											<div class="tbl_wrap">
												<dl class="section">
													...
													<dd>
														<input type="text" id="staff_id" name="staff_id" style="width:90%;" maxlength="20" />
													</dd>
													<dt><label for="t_login_id">메뉴 ID :</label></dt>
													<dd>
														<input type="text" id="menu_id" name="menu_id" style="width:90%;" maxlength="20" />
													</dd>
												</dl>
												<dl class="section">
													<dt><label for="admin_typ">구분 :</label></dt>
													<dd>
														<select style="width:90%" id="his_type" name="his_type">
												...
												<dl class="section">
													<dt><label for="a7">변경 일자 :</label></dt>
													<dd class="cols">
