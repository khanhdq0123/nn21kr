...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('[name="board_staff_yn"]').on('click', function () {

				var vStaff_yn  = $('input[name="board_staff_yn"]:checked').val();
				var vVendor_nm = $('#vendor_nm').val();
				
				if(vStaff_yn == 'Y' && vVendor_nm == ''){
					toast('소속(업체명)을 선택하고, 1:1 문의 담당자 여부를 \'Y\'로 선택해 주세요');
					$("input[name=board_staff_yn]").val(['N']);
					//toast($('input[name="board_staff_yn"]:checked').val());
					return;
				}else{
					$.ajax({
						type: "POST",
						url: "/system/staff/vendorIDCountJson?type=<c:out value="${param.type}"/>",
						data: $('#gForm').serialize(),
						dataType: 'json',
						contentType: 'application/x-www-form-urlencoded; charset=utf-8',
						success: function(data) {
							if(data.data != null){
								toast(data.data.LOGIN_ID + '가 이미 1:1 문의 담당자로 지정되어 있습니다.');
								$("input[name=board_staff_yn]").val(['N']);
							}
						},
						error: function(jqXHR, textStatus, errorThrown) {
...
// 			$("#job_cd option:eq(1)").prop("selected",true);

			$("#job_cd").val("11402").prop("selected", true);
			$("#position_cd").val("11301").prop("selected", true);
			
			$("#staff_enm").val("kim young hwan");
			$("#staff_nm").val("김영환");
			
			$("#mobile_txt").val("010-4708-4073");
			$("#email_txt").val("yh98kim@gmail.com");
			
			$("#login_id").val("test");
			$("#passwd_txt").val("seffice1004^^");
			$("#passwd_txt2").val("seffice1004^^");
...
<div class="layerpop_inner" id="#">
	...
	<h1 class="ly_header" onclick="fnTest();">사용자 등록 상세</h1>
	<!-- .ly_body -->
	<div class="ly_body">
...
<c:if test="${IsMaster eq 'Y'}">				
		...
		<c:if test="${sessionScope['STAFFINFORMATION']['staff_typ'] eq '3'}">
			<input type="hidden" id="staff_typ" name="staff_typ" value="3" />
		</c:if>
				...
				<caption>운영자 등록 상세 페이지</caption>
					...
					<tbody>
						...
						<tr>
							...
							<td>							
								<c:choose>
									<c:when test="${_TYP eq 'modify'}">
										<input type="text" id="login_id" name="login_id" value="<c:out value='${_DATA.login_id}'/>" style="width:150px;" maxlength="30" readonly class="readonly"  />
									</c:when>
									<c:otherwise>
										<input type="text" id="login_id" name="login_id" value="" style="width:150px;" maxlength="30"   />
									</c:otherwise>
								</c:choose>
								<c:if test="${_TYP eq 'add'}">
						...
						<tr>
							...
							<td>
								<input type="text" id="vendor_nm" name="vendor_nm" value="${vendor_info.vendor_nm}" readonly="readonly" style="width:90%;background:#eee;" />
							</td>
							<c:if test="${ IsMaster eq  'Y'}">
						...
						<tr>
							...
							<th scope="row"><span class="starMark">필수입력 항목입니다.</span></th>
							<td>
