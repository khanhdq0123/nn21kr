...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.tagName.toUpperCase() !== 'TABLE' || this.id == null) {
			toast("Element is not a table or has no id!");
			return;
		}
		if(document.documentMode !== undefined ) { // IE only
			if(document.documentMode <= 5) {
				toast("Grid can not be used in this ('quirks') mode!");
				return;
			}
		}
            ...
            return this.each(function(){
                ...
                var xmlConvert = function (xml,o) {
                    ...
                    if(xmlJsonClass.xml2json && $.jgrid.parse) {
                        ...
                        toast("xml2json or parse are not present");
                    }
                };
		...
		if($.ui) {
			if ($.ui.multiselect && $.ui.multiselect.defaults) {
				if (!$.jgrid._multiselect) {
					...
					toast("Multiselect plugin loaded after jqGrid. Please load the plugin before the jqGrid!");
					return;
				}
...
var xmlJsonClass = {
	...
	toObj: function(xml) {
		...
		if (xml.nodeType === 1) {
			...
			toast("unhandled node type: " + xml.nodeType);
		}
		return o;
	},
