...
a.groups.length;c++){0<d&&e&&u.or();try{b(a.groups[c])}catch(l){toast(l)}d++}e&&u.orEnd()}if(null!=a.rules)try{(f=a.rules.length&&"OR"===a.groupOp.toString().toUpperCase())&&u.orBegin();for(c=0;c<a.rules.length;c++)k=a.rules[c],g=a.groupOp.toString().toUpperCase(),t[k.op]&&k.field&&(0<d&&g&&"OR"===g&&(u=u.or()),u=t[k.op](u,g)(k.field,k.data,h[k.field])),d++;f&&u.orEnd()}catch(p){toast(p)}}var c=d.p.multiSort?[]:"",e=[],f=!1,h={},g=[],k=[],l,p,n;if(a.isArray(d.p.data)){var m=d.p.grouping?d.p.groupingView:
...
var b=c,d=a,e,f;0<=d?(e=b.slice(),f=e.splice(d,Math.max(b.length-d,d)),d>b.length&&(d=b.length),e[d]=a,c=e.concat(f)):c=b});b.done&&b.done.call(f,c);f.jqGrid("setGridWidth",f[0].p.tblwidth,f[0].p.shrinkToFit)},cleanup:function(a){c(b.dlog,g,"destroy");c(b.msel,d,"destroy");g.remove();a&&b.done&&b.done.call(f)},msel_opts:{}},a.jgrid.col,b||{});if(a.ui&&a.ui.multiselect&&a.ui.multiselect.defaults){if(!a.jgrid._multiselect){toast("Multiselect plugin loaded after jqGrid. Please load the plugin before the jqGrid!");
