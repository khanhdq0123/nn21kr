...
nhn.husky.SE2M_BGColor = jindo.$Class({
	...
	$ON_APPLY_BGCOLOR : function(sBGColor, sFontColor){
		if(!this.rxColorPattern.test(sBGColor)){
			toast(this.oApp.$MSG("SE_Color.invalidColorCode"));
			return;
		}
...
nhn.husky.SE2M_FontColor = jindo.$Class({
	...
	$ON_APPLY_FONTCOLOR : function(sFontColor){
		if(!this.rxColorPattern.test(sFontColor)){
			toast(this.oApp.$MSG("SE_FontColor.invalidColorCode"));
			return;
		}
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
	...
	$ON_APPLY_HYPERLINK : function(){
		...
		if(!this._validateTarget()){
			toast(this.oApp.$MSG("SE_Hyperlink.invalidTarget"));
			return;
		}
		...
		if(this._validateURL(sURL)){
			...
			toast(this.oApp.$MSG("SE_Hyperlink.invalidURL"));
			this.oLinkInput.focus();
		}
	},
...
nhn.husky.SE2M_LineHeightWithLayerUI = jindo.$Class({
	...
	_setLineHeightAndCloseLayer : function(sValue){
		...
		if(nLineHeight>0){
			...
			toast(this.oApp.$MSG("SE_LineHeight.invalidLineHeight"));
		}
...
//		this.welDropdownLayer.offset(this.htOffsetPos.top-htScrollXY.y, this.htOffsetPos.left-htScrollXY.x);
	...
	$ON_FIND : function(){
		...
		switch(this.oFindReplace.find(sKeyword, false)){
			case 1:
				toast(this.oApp.$MSG("SE_FindReplace.keywordNotFound"));
				oSelection.select();
				break;
			case 2:
				toast(this.oApp.$MSG("SE_FindReplace.keywordMissing"));
				break;
		}
	},
	...
	$ON_REPLACE_ALL : function(){
		...
		if(iReplaceAllResult === 0){
			toast(this.oApp.$MSG("SE_FindReplace.replaceKeywordNotFound"));
			oSelection.select();
			this.oApp.exec("FOCUS");
		}else{
			if(iReplaceAllResult<0){
				...
				toast(this.oApp.$MSG("SE_FindReplace.replaceAllResultP1")+iReplaceAllResult+this.oApp.$MSG("SE_FindReplace.replaceAllResultP2"));
				oSelection = this.oApp.getEmptySelection();
				oSelection.select();
				this.oApp.exec("FOCUS");
			}
...
//		var sBookmarkID = oSelection.placeStringBookmark();
		...
		if(this._isExceedMaxDepth(oFormattingNode)){
			toast(this.oApp.$MSG("SE2M_Quote.exceedMaxCount").replace("#MaxCount#", (this.nMaxLevel + 1)));
...
function Shortcut(sKey,sId){
	...
	toast(sId+"는 반드시 string이거나  없어야 됩니다.");
};
	...
	if (this._instance==null) {
		...
		this._instance = new (function(){
			...
			this.openWindow = function(oWinOpt) {
				...
				if(op.oApp == null) {
					toast("팝업 요청시 옵션으로 oApp(허스키 reference) 값을 설정하셔야 합니다.");
				}
		...
		if(bRulerUse){
			...
			if(welWysiwygBody){
				...
				if(width > nEditorWidth){
					bPossible = false;
					toast(this.oApp.$MSG("SE2M_ImgSizeRatioKeeper.exceedMaxWidth").replace("#EditorWidth#", nEditorWidth));
				}
			}
		}
		return bPossible;
