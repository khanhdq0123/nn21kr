...
if (cssquery.test(oEl, 'div, p, [align=center]')) toast('해당 조건 만족');// oEl 이 div 태그 또는 p 태그, 또는 align=center 인 엘리먼트인지
if (cssquery.test(oEl, 'div, p, [align=center]')) toast('해당 조건 만족');
...
//		throw new Error("Check your flash file!. Unload flash on a page.");
		...
		// toast("Check your flash file!. Unload flash on a page.");
	},5000);
	jindo.$Ajax._checkFlashLoad = function(){}
}
/**
 ...
 * @example
	...
	$Element("parent").delegate("click",
		...
		function(eEvent){	// 콜백 함수
			toast("odd 클래스를 가진 li가 클릭 될 때 실행");
		});
 * @example
	<ul id="parent">
		<li class="odd">1</li>
	...
	$Element("parent").delegate("click",
		...
		function(eEvent){			// 콜백 함수
			toast("클릭한 엘리먼트의 innerHTML이 2인 경우에 실행");
		});
...
jindo.$Element.prototype._bind = function(sEvent,vFilter,fpCallback){
	...
	if(_aDataOfEvent){
		...
		toast("check your delegate event.");
	}
}
/**
...
for(var i=0; i<2;i++){
aTmp[i] = $Fn(function(nTest){toast(nTest);}, this).bind(i);
}
...
var MainClass = $Class({
	...
	func1 : function(){
		toast(this);// this는 MyClass 를 의미한다.
	},
	func2 : function(){
		toast(this);// this는 MainClass 를 의미한다.
	}
})
function init(){
	var a = new MainClass();
...
toast('가로 : ' + size.width + ' / 세로 : ' + size.height);
...
toast('가로 : ' + size.left + ' / 세로 : ' + size.top);
* @since 1.3.5
...
<script type="text/javascript">
	...
	toast(allstr == 'ONE=1&TWO=2&THREE=3_2&FOUR=4_3'); // true

	var str = form.serialize('ONE', 'THREE');
	toast(str == 'ONE=1&THREE=3_2'); // true
</script>
...
<script type="text/javascript">
	...
	toast(one_value === '1'); // true

	var two_value = form.value('TWO');
	toast(two_value === undefined); // true

	form.value('TWO', 2);
	toast(two_value === '2'); // true
