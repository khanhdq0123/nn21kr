	$(document).ready(function() {			
			...
			jQuery("#list1").jqGrid({
				url:'/ad/ex/ADEX01/search',
//				datatype: "json",				   
				datatype: "local",				   
				colNames:['exch_no','적용일', 'USD($)', 'CNY(￥)', 'VND(₫)', 'JPY(Y)', 'HKD(HK$)','EUR(€)','IDR(Rp)','PHP(₱)','THB(฿)'],
				colModel:[
					{name:'exch_no',	index:'exch_no',	hidden:true},
					{name:'app_dt',		index:'app_dt',		width:90,	align:"center" , sortable:true, editable:true,editoptions:{dataInit:function(e) {$(e).datepicker();}}}, //적용일
...
//			});
			...
			function linkFormatter(cellVal, options, rowObj) {
				var linkURL = '<a href="javascript:CenterOpenWindow(\'/ad/ex/ADEX01/detail?exch_no=' + rowObj['exch_no'] + '\', \'open\', \'1200\', \'800\', \'scrollbars=yes, status=no\');">'+cellVal+'</a>';										
				return linkURL;
			}
		...
		//신규등록
		function fn_f9(){
			CenterOpenWindow('/ad/ex/ADEX01/popup', 'open', '500', '550', 'scrollbars=yes, status=no');
		}
		
		//저장
		function fn_f10(){
			saveGridSelected("list1", 'selected', 'save', "/ad/ex/ADEX01/update");
		}
