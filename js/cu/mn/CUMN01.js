	$(document).ready(function() {
		...
		// 그리드 - 프로토타입 
		jQuery('#list1').jqGrid({
			url: '/cu/mn/CUMN01/selectManualList',
			datatype: 'local',
		   	colNames: ['manual_no','manual_typ','등록일시','제목','등록자'
			],
			colModel: [
	            { name: 'manual_no', 	index: 'manual_no', 		hidden:true},
	            { name: 'manual_typ', 	index: 'manual_typ',  	hidden:true},
...
// 	        }
		...
		function linkFormatter(cellVal, options, rowObj) {
			var linkURL = '<a href="javascript:CenterOpenWindow(\'/cu/mn/CUMN01/popup1' 
					+ '?manual_no=' + rowObj['manual_no']
...
//신규등록
function fn_f9(){
	CenterOpenWindow('/cu/mn/CUMN01/popup?node=new', 'open', '1000', '640', 'scrollbars=yes, status=no');
}

//저장
function fn_f10(){
	saveGridSelected("list1", 'selected', 'save', "/cu/mn/CUMN01/update");
}

//삭제
function fn_f11(){
	saveGridSelected("list1", 'selected', 'delete', "/cu/mn/CUMN01/delete");
}
