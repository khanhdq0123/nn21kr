	...
	$(document).ready(function() {
		...
		// 그리드 - 프로토타입 
		jQuery('#list1').jqGrid({
			url: '/ve/or/VEOR01/search',
			datatype: 'local',
		   	colNames: ['고객번호', '회사명',	'수취인', '예약 요약','선적일', '품목', 'CT', 'Kg', 'CBM'
			],
			colModel: [
	            { name: 'send_cd', 			index: 'send_cd', 			align: 'center', 	sortable: true, width: '90'},
	            { name: 'send_nm', 		index: 'send_nm', 		align: 'center', 	sortable: true, width: '150'},
	            { name: 'receive_nm', 	index: 'receive_nm', 	align: 'left',		sortable: true, width: '150'},
	            { name: 'summury_info',	index: 'summury_info',	align: 'center',	sortable: true, width: '350', formatter : linkFormatterOrd},
	            { name: 'shipment_dt', 			index: 'shipment_dt', 			align: 'center', 		sortable: true, width: '100', formatter: "date", formatoptions: { newformat: " Y/m/d" }},
	            { name: 'item_nm', 			index: 'item_nm', 			align: 'center', 		sortable: true, width: '150'},
...
//			rowList: [20, 50, 100, 1000],
	...
	function linkFormatterOrd(cellVal, options, rowObj) {
		...
		linkURL += rowObj['start_dt'] + ' (' + rowObj['send_region_nm'] + ')';
		linkURL += ' ~' + rowObj['arrive_dt'] + ' (' + rowObj['receive_region_nm'] + ')';
		linkURL += '</a>';		
		return linkURL;
	}
	...
	//신규등록
	function fn_f9(){
		var url = "/ve/or/VEOR01/popup1?ord_typ=";
