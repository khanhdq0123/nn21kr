...
//						$('table.ui-jqgrid-ftable td:eq(3)').html('<input type="button" class="btn btn-primary btn-xs" value="calc" id="btnCalcItem" onclick="fnCalcItem();">').css("text-align", "right");
				...
				// 그리드 - 프로토타입 
				jQuery('#recentOrdTtemList').jqGrid({
					url: '/order/selectRecentOrdItemList',
					datatype: 'local',
				   	colNames: ['tp_no','Date', 'item name'
					],
					colModel: [
			            { name: 'tp_no', 			index: 'tp_no', 			align: 'center', 	sortable: true, width: '70', hidden:true},
			            { name: 'date_to_from', 			index: 'date_to_from', 			align: 'center', 	sortable: true, width: '70', cellattr: catMerge},
			            { name: 'item_nm', 			index: 'item_nm', 			align: 'left', 		sortable: true, width: '140'},
					],
					autowidth: true,
					height: 160,
					mtype: 'post',
		...
		$("#recentOrdTtemList").jqGrid('setGridParam',{datatype:'json'}).trigger("reloadGrid");

		
		//$("#recentOrdTtemList").trigger("reloadGrid");
