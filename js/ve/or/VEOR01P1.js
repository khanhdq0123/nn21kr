...
//								});
	...
	function fnEvenBind(){
		$(".select-layer1 ul li").on("click", function(){
			...
			var oldCdS = $("#start_vendor_no").val();
			var oldCdC = $("#arrive_vendor_no").val();
			...
			var typ = $("#selectVendorLayerPop").attr("data-typ");


			if(typ == 'S' && cd == oldCdS) return;
			if(typ == 'C' && cd == oldCdC) return;
			...
			if(typ == 'S'){
				...
				$("#receive_cc_comp_no").val("");
				$("#receive_cc_comp_nm").val("");
			}
...
// 					toast('먼저 운송구분1을 선택하세요.');
	...
	function fnSelectLayer1(_tit, _layerId, _retCdId, _retNmId){
		...
		if(_retCdId == 'start_vendor_no'){
			$layer.attr('data-typ','S');
		} else {
			$layer.attr('data-typ','C');
		}
	...
	function fnSelectLayer2(_tit, _layerId, _retCdId, _retNmId){
		...
		var _start_vendor_no = $("#start_vendor_no").val();
		var _arrive_vendor_no = $("#arrive_vendor_no").val();
		
		var _send_no = $("#send_no").val();
		var _receive_no = $("#receive_no").val();


		if(_start_vendor_no == '' && _arrive_vendor_no == ''){
			toast('지사를 먼저 선택 하세요.');
			return false;
		}

		var _vendor_no = (_retCdId == 'send_no') ? _start_vendor_no : _arrive_vendor_no;

		var data = {}
		data.vendor_no = _vendor_no;
		data.vendor_typ = '3';

		$.ajax({
			type: "POST",
			url: '/partner/selectSubCompList',
			data: data,
			dataType: 'json',
			contentType: 'application/x-www-form-urlencoded; charset=utf-8',
			success: function(data) {
				
				$("#" + _layerId + " .contents ul li").empty();
				...
				if(data.list.length > 0) {
					...
					$(data.list).each(function(i,d){
						...
						_html += '<li cd="' + d.vendor_no + '" nm="[' + d.vendor_no + '] ' + d.vendor_nm + '"><b>[' + d.vendor_no + '] ' + d.vendor_nm + '</b> : ' + d.basadd_txt + '</li>';
					});

					$("#" + _layerId + " .contents ul").append(_html);
					...
					$(".select-layer1 ul li").on("click", function(){
						...
						$('#' + retCdId).val(cd);
						$('#' + retNmId).val(nm);
						fnCloseLayer(_layerId);
						
						if(_retCdId == 'send_no' && cd == _send_no) return;
						if(_retCdId == 'receive_no' && cd == _receive_no) return;
					...
					$("#" + _layerId + " .contents ul").append("<li style='text-align:center;'>등록된 데이터가 없습니다.</li>");
				}
			},
	...
	function fnSelectLayer3(_tit, _layerId, _retCdId, _retNmId){
		...
		var _vendor_no = (_retCdId == 'send_trade_comp_no' || _retCdId == 'send_cc_comp_no' ) ? _send_no : _receive_no;

		var _vendor_typ = '';
		
		if(_retCdId == 'send_trade_comp_no' || _retCdId == 'receive_trade_comp_no'){
			_vendor_typ = '7';
		}
		
		if(_retCdId == 'send_cc_comp_no' || _retCdId == 'receive_cc_comp_no'){
			_vendor_typ = '8';
		}

		var data = {}
		data.vendor_no = _vendor_no;
		data.vendor_typ = _vendor_typ;
